...
| **ソースコード・設計書** | **GitHub MCPTool (Remote MCP)** → Agent Service のツールとしてリアルタイム検索・取得 |
...
![アーキテクチャ図](docs/images/architecture.svg)
...
Foundry Agent Service (Prompt Agent)
 ...
 │
 └── GitHub MCPTool (サーバー側実行 — Remote MCP Server)
       ├─ search_code         → GitHub Code Search API
       ├─ get_file_contents    → GitHub Contents API
       └─ (その他 GitHub MCP ツール)
```
...
```
Step 1: Bicep デプロイ
  → AI Services (Foundry) + Foundry Project + AI Search + Storage + ロール割り当て
...
デプロイされるリソース:
- Azure AI Services (Foundry — gpt-4o + text-embedding-3-large を統合管理)
...
| 2 | GitHub MCPTool (Remote MCP) 構築 | [docs/module2-github-mcp.md](docs/module2-github-mcp.md) |
...
│   ├── config.py                   # 設定管理
│   ├── tools/
│   │   └── github_tools.py         # GitHub API ユーティリティ（参考コード）
│   ├── foundry_iq/
